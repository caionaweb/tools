...
rhel5 ()	{
	...
	case $vsysarch in
			32) # 32 bit Squid configuration
					wget -O /etc/squid/squid.conf https://raw.githubusercontent.com/caionaweb/tools/master/squid-install/spi-rhel5632.conf --no-check-certificate;;
			64) # 64 bit Squid configuration
					wget -O /etc/squid/squid.conf https://raw.githubusercontent.com/caionaweb/tools/master/squid-install/spi-rhel5664.conf --no-check-certificate;;
	esac
...
rhel7 ()	{
	...
	wget -O /etc/squid/squid.conf https://raw.githubusercontent.com/caionaweb/tools/master/squid-install/spi-rhel7.conf --no-check-certificate
	# Creating empty blacklist.acl file for further blacklisting entries
	touch /etc/squid/blacklist.acl
	# Restarting Squid and enabling its service
...
deb ()	{
	...
	# Downloading Squid configuration
	wget -O /etc/squid3/squid.conf https://raw.githubusercontent.com/caionaweb/tools/master/squid-install/spi-debian.conf --no-check-certificate
	# Creating empty blacklist.acl file for further blacklisting entries
...
deb8 ()	{
	...
	htpasswd -c /etc/squid/passwd $usrn
	# Downloading Squid configuration
	wget -O /etc/squid/squid.conf https://raw.githubusercontent.com/caionaweb/tools/master/squid-install/spi-jessie.conf --no-check-certificate
	# Creating empty blacklist.acl file for further blacklisting entries
	touch /etc/squid/blacklist.acl
	touch /etc/squid/whitelist.acl
	# Restarting Squid and enabling its service
...
ubt ()	{
	...
	htpasswd -c /etc/squid3/passwd $usrn
	# Downloading Squid configuration
	wget -O /etc/squid3/squid.conf https://raw.githubusercontent.com/caionaweb/tools/master/squid-install/spi-ubuntu.conf --no-check-certificate
...
fed () {
	...
	case $vsysarch in
		32) # 32 bit Squid configuration
			wget -O /etc/squid/squid.conf https://raw.githubusercontent.com/caionaweb/tools/master/squid-install/spi-fedora32.conf --no-check-certificate;;
		64) # 64 bit Squid configuration
			wget -O /etc/squid/squid.conf https://raw.githubusercontent.com/caionaweb/tools/master/squid-install/spi-rhel7.conf --no-check-certificate;;
	esac
	# Creating empty blacklist.acl file for further blacklisting entries
